            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if (data.resultCode == 1) {
                        console.log('红包雨，获得' + data.data.goldCoinAmt / 100 + '元\n')
                        $.message +=
                            '红包雨，获得' + data.data.goldCoinAmt / 100 + '元\n'
                    }
                    if (data.resultCode == 0) {
                        console.log(data.data.errorDesc + '\n');
                        $.message +=
                            '【红包雨福利】：' + data.data.errorDesc + '\n';
                    }
                } catch (e) {
                    $.logErr(e, resp);
                } finally {
