...
const inviteCodes = [
  'cgxZ-scH8TplPoJf3Ik_O1Yn0wIgOuL1FLudvFEIQKzabFxvRpCh4j8'
];
const pkInviteCodes = [
  'IgNWdiLGaPYPTIsciLp9Y_ylailacpoNwb2gortOSsS0VF34fsQiBf9iSihJqA'
]
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
    ...
    $.get({url: `http://jd.turinglabs.net/api/v2/jd/nian_pk/read/0/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data) {
            console.log(`随机取0个PK助力码放到您固定的互助码后面(不影响已有固定互助)`)
            data = JSON.parse(data);
          }
        }
      } catch (e) {
